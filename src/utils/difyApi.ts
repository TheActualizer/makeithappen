...
export async function sendMessageToDify(message: string, conversationId: string) {
  console.log('DifyAPI: Starting API call', { message, conversationId });
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('DifyAPI: Error response:', errorData);
      throw new Error(`Dify API error: ${errorData.message || response.statusText}`);
    }

    const responseData = await response.json();
    console.log('DifyAPI: Successful response:', responseData);
    ...
    console.error('DifyAPI: Error in API call:', error);
    throw error;
  }
}